...
// ---------------------------------------------------------------------------
// An M998 HMMWV driven through one cycle of motion: a truck under way on a rough track.
//
// Six things move, and each wanted the model split differently from the five groups a static review needs.
...
//
// Transparency is also why this file has a cab interior at all. Filling the cab with a solid block up to the roof is enough behind opaque windows; seen through glass it is a black wall, and the windows read as paint. The block therefore stops just under the window line and what stands above it -- dash, tunnel, seat backs, steering wheel -- is modelled.
// ---------------------------------------------------------------------------
...
// Mirror face. An earlier version gave the wing mirrors the glass material, harmless while glass was opaque and not now: a see-through mirror is the one pane in the model that should never be see-through.
...
static void MakeTextures(void)
{
    ...
    if (gGlassShader.id == 0) {
        TraceLog(LOG_WARNING, "humvee: glass shader failed to build, windows will render opaque");
    }
    gDustShader = LoadShaderFromMemory(GLASS_VS, DUST_FS);
    if (gDustShader.id == 0) {
        TraceLog(LOG_WARNING, "humvee: dust shader failed to build, puffs will render as opaque pebbles");
    }
}
...
// A static build of this truck needs five, one per review part. This file has twenty-six, because a part that has to move alone has to be its own mesh.
//
...
static void GroupRegister(Group *g)
{
    ...
    else TraceLog(LOG_ERROR, "humvee: more than %d groups", MAX_GROUPS);
}
...
// The HMMWV's screen stands close to upright: measured off references/humvee/ref_07.jpg (side elevation) at 6 degrees, ref_05 at 9 and ref_01 at 11 once each view's yaw is divided out.
...
// Measured off references/humvee/ref_06.jpg, a near-side view of the cab: the side glass runs 460 px of the door's 1080, so 0.426 of the leaf.
#define BELT_Y        1.273f
#define GLASS_Y0      1.303f
#define GLASS_Y1      1.735f
// Top of the cab's solid core. It used to run to the roof; it now stops just under the belt line so the glass has an interior behind it rather than a black wall.
...
// Measured off references/humvee/ref_10.jpg, a near-orthographic side elevation of this variant, scaled by the 3.30 m wheelbase at 403 px/m and anchored vertically on the axle centre. Reading the top line back into these coordinates gives 1.208 m at z = 0.85 falling to 1.064 m at z = 2.03, and the local slope over that run goes 4.0, 5.2, 8.3, 11.7, 13.8 degrees: it starts falling at the cowl and steepens the whole way, with no flat section and no crease anywhere.
...
// A first difference is enough here and a long way from the trap a second difference in float sets: these curves are low-order polynomials sampled at 4 mm on values near 1, so the quantisation lands around 1e-5 against slopes of order 0.1.
...
static void BuildFront(void)
{
    ...
    // 20 mm inboard of the fenders so those shut lines read as gaps rather than as a single moulded lump, and 10 mm down into the bay's roof rather than flush with it, since two faces on exactly the same plane is a defect an earlier version of this truck shipped four of.
    ...
    // Proportioned off references/humvee/ref_09.jpg, a plan view of the hood, and checked head-on against ref_03.jpg: it spans a little over 40 per cent of the hood's width and sits in its forward half.
    {
    ...
    // The main lamp is 0.178 across: measured off references/humvee/ref_03.jpg, where it spans 110 px against the 1310 px that carry the vehicle's 2.16 of width.
...
static void BuildCab(void)
{
    ...
    // This used to run to the roof, which is fine while the glass is opaque and useless once it is not; everything the eye can now reach through a window is built as interior instead.
...
// That is measured, not assumed: references/humvee/ref_04.jpg looks straight into the windscreen and shows both pivots on the header, both arms hanging down and both leaning the same way, towards the vehicle's right. Mapping that view's pane edges onto x puts each pivot at |x| = 0.46 and each blade tip about 0.32 outboard of it. An earlier version built a short arm pivoting near the cowl instead, which is upside down.
...
static void AntennaSetBend(float bendX, float bendZ)
{
    ...
    if (b->vertexCount != verts || b->triangleCount != tris || b->vertices != base) {
        TraceLog(LOG_ERROR, "humvee: antenna rebuild changed the buffer (%d/%d vertices, %s)",
...
// 10 mm short of the hub carrier's outer face at 0.800, so the shaft's end cap is buried in it rather than flush with it. Flush is how an earlier version of this truck ended up with four joints on exactly coincident planes.
...
// Second differences in float are the trap this project has already hit once, so the step is 10 ms rather than the millisecond that put an earlier model's swept-curve frame normal into the noise: at this amplitude the quantisation shows up around 1e-5 m/s2 against signals of a few m/s2.
...
// One tier of the average of the two reads as ground fog, which is what renders/humvee/v3/critique.md called this before the split.
...
// Checks
//
// A pose can be wrong without looking wrong in any one frame, which is the whole reason a linkage is walked at build time instead of argued to line up. These do the same for the four claims this file makes that a still cannot settle.
// ---------------------------------------------------------------------------
...
static void CheckDoorClearsScreen(void)
{
    ...
    if (worst < 0.020f) {
        TraceLog(LOG_WARNING, "humvee: door leading edge clears the windscreen by only %.3f m at y = %.3f", worst, worstY);
    }
}
...
static void CheckGroundContact(void)
{
    ...
    TraceLog(LOG_INFO, "humvee: tyre contact runs from %+.4f m to %+.4f m, airborne for %.2f s of %.1f, peaking at phase %.3f",
             deepest, highest, airborne, CYCLE, peakT / CYCLE);
    if (deepest < -0.001f) {
        TraceLog(LOG_WARNING, "humvee: a tyre reaches %.4f m through the ground plane", deepest);
    }
}
...
static void CheckWiperSweep(void)
{
    ...
    TraceLog(LOG_INFO, "humvee: blade tip sweeps x %.3f to %.3f, down to %.2f of the screen",
             nearest, outermost, lowest);
    if (nearest < 0.040f || outermost > CAB_IN || lowest < 0.0f) {
        TraceLog(LOG_WARNING, "humvee: wiper leaves its pane (x %.3f..%.3f, f down to %.2f)",
                 nearest, outermost, lowest);
    }
}
...
static void CheckLoopCloses(void)
{
    ...
    TraceLog(LOG_INFO, "humvee: loop closes to %.2e, antenna to %.2e rad, wheel to %.2e of a turn", worst, bend, slip);
    if (worst > 1e-4f || slip > 1e-4f || bend > 1e-3f) {
        TraceLog(LOG_WARNING, "humvee: pose does not close over the cycle (%.2e, antenna %.2e, wheel %.2e turns)", worst, bend, slip);
    }
}

// Claim: each half shaft's outer end stays inside the hub carrier it drives, at every point of the travel. This is the joint renders/humvee/v1/critique.md found open, so it is measured rather than argued.
static void CheckHalfShaft(void)
{
    ...
    TraceLog(LOG_INFO, "humvee: half shaft end sits %.3f m inside the hub carrier at worst", worst);
    if (worst < 0.0f) {
        TraceLog(LOG_WARNING, "humvee: half shaft end leaves the hub carrier by %.3f m", -worst);
    }
}

// Claim: the front's top skin has the shape the reference has.
// The six readings are the top line of references/humvee/ref_10.jpg, taken at 403 px/m off the 3.30 m wheelbase and anchored vertically on the axle centre, converted into this file's coordinates. Keeping them here rather than in a comment means a later edit to the curve is told how far it has drifted rather than being trusted.
...
static void CheckHoodProfile(void)
{
    ...
    TraceLog(LOG_INFO, "humvee: hood skin sits %+.3f m against ref_10, shape within %.3f m of it",
             mean, hi - lo);
    if (hi - lo > 0.030f) {
        TraceLog(LOG_WARNING, "humvee: hood skin is the wrong shape, not merely offset: %.3f m of spread", hi - lo);
    }
}
...
static void CheckDust(void)
{
    ...
    Update(0.0f);
    TraceLog(LOG_INFO, "humvee: dust emits %d puffs of %d, %d gated off airborne tyres, at most %d alive, reaching %.2f m past the tail and %.2f m up",
             emitters, DUST_COUNT, gated, mostAlive, far, high);
    if (gated == 0) {
        TraceLog(LOG_WARNING, "humvee: no dust puff was gated by wheel contact, so the jump leaves no gap in the plume");
    }
}
...
static void CheckSuspension(void)
{
    ...
    TraceLog(LOG_INFO, "humvee: suspension travels up to %.3f m, coil to %.0f%% of free length",
...
const Scene SCENE = {
    .name = "humvee",
    .description =
        "M998 HMMWV cargo/troop carrier under way on a rough track: measured\n"
        "geometry, a cab opened up behind transparent glass, and one four-second\n"
        "loop of motion.\n"
        "\n"
        ...
        "across the panes; an earlier version had a short arm pivoting near the cowl\n"
        "instead. The front\n"
        ...
        "\n"
        "Surfacing: world-space planar UVs repeating every 1.60 m so patterns\n"
