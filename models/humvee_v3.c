...
// ---------------------------------------------------------------------------
// This is humvee_v2's geometry carrying procedurally generated surface maps and lit lamps.
// The maps are built in code rather than loaded, so the model stays a single self-contained file with no asset directory beside it.
//
// Two things here are not in humvee_v2.
// Every vertex gets a world-space planar UV, so the camouflage flows across panel joins instead of restarting on each face.
// The lamp lenses and their haloes skip the lighting shader, which is what makes them read as emitting rather than as pale paint: raylib's default shader returns the texel unshaded, so a near-white lens sits at full brightness while the lit bodywork around it does not.
// ---------------------------------------------------------------------------
...
typedef enum {
    MAT_BODY, MAT_DARK, MAT_METAL, MAT_GLASS, MAT_LAMP, MAT_TAIL, MAT_GLOW_W, MAT_GLOW_R, MAT_COUNT
} MatId;
...
// The two glow materials are the exception: they share one white radial falloff and take their colour from the tint.
...
// The lamps and their haloes are drawn without the lighting shader. raylib's default shader returns texel * tint unshaded and ungamma'd, so these sit at their literal brightness while everything lit around them is knocked down by the light dot product.
static const bool MAT_UNLIT[MAT_COUNT] = {
    [MAT_LAMP] = true, [MAT_TAIL] = true, [MAT_GLOW_W] = true, [MAT_GLOW_R] = true,
};

// The haloes are drawn last and summed into the frame rather than compared against it.
static const bool MAT_ADDITIVE[MAT_COUNT] = {
    [MAT_GLOW_W] = true, [MAT_GLOW_R] = true,
};
...
static Texture2D MakeGlassTexture(void)
{
    ...
    for (int y = 0; y < S; y++) {
        for (int x = 0; x < S; x++) {
            float u = (float)x / (float)S, v = (float)y / (float)S;
            px[y * S + x] = Shade((Color){ 13, 19, 24, 255 }, (Fbm(u, v, 4, 91u, 2) - 0.5f) * 10.0f);
        }
    }
    return Upload(img, TEXTURE_WRAP_REPEAT);
}
...
// Both glow materials share one texture, so only the one that owns it may unload it.
static void UnloadTextures(void)
{
    for (int m = 0; m < MAT_COUNT; m++) {
        if (m == MAT_GLOW_R) continue;
...
static void GroupFinish(Group *g, const char *name)
{
    ...
    for (int m = 0; m < MAT_COUNT; m++) {
        ...
        // Leaving the lamps on raylib's default shader is what lights them: it returns the texel unshaded, so the lens keeps its full brightness while the bodywork beside it is dimmed by the light dot product.
...
static void GroupDraw(const Group *g)
{
    ...
    for (int m = 0; m < MAT_COUNT; m++) {
        if (!g->has[m]) continue;
        if (MAT_ADDITIVE[m]) { anyGlow = true; continue; }
        DrawModel(g->model[m], Vector3Zero(), 1.0f, WHITE);
    }
    if (!anyGlow) return;

    // Haloes go last, summed into the frame, with depth writes off.
    // Left writing depth, the nearer of two overlapping discs would occlude the further one and the pair would read as a step rather than as one brighter patch.
    // Depth testing stays on, so a halo is still hidden by bodywork in front of it.
    BeginBlendMode(BLEND_ADDITIVE);
    rlDisableDepthMask();
    for (int m = 0; m < MAT_COUNT; m++) {
        if (g->has[m] && MAT_ADDITIVE[m]) DrawModel(g->model[m], Vector3Zero(), 1.0f, WHITE);
    }
    rlEnableDepthMask();
    EndBlendMode();
}
...
static void BuildFront(void)
{
    ...
    GlowDisc(&g->b[MAT_GLOW_W], 1.000f, 1.025f, 2.248f, 0.115f, 1.0f);
...
{
    // Before any group is finished: GroupFinish hands each material its map, so they have to exist by then.
    MakeTextures();
    BuildHull();
    BuildFront();
    BuildCab();
...
const Scene SCENE = {
    ...
    .description =
        ...
        "humvee_v2's geometry, surfaced with procedurally generated maps and with the\n"
        "lamps lit.\n"
        "\n"
        "Surfacing: every vertex carries a world-space planar UV, projected onto the\n"
        "two axes its normal is weakest in and repeating every 1.60 m, so patterns run\n"
        "across panel joins rather than restarting per face. Six maps, all built in\n"
        "code from tiling value noise: NATO three-colour woodland camouflage over the\n"
        "bodywork (four colours thresholded out of one noise field, with a fine CARC\n"
        "speckle), rubber for the tyres and dark trim, mottled metal, near-flat glass,\n"
        "and two lens maps that are hot in the middle and fall off to the rim.\n"
        "\n"
        "Lamps: the lens and halo materials skip the lighting shader, so they render\n"
        "unshaded at full brightness while the lit bodywork around them does not. Each\n"
        "lamp also carries a halo disc, a radial alpha falloff drawn additively with\n"
        "depth writes off, 2 mm off its lens face and about twice its radius. Headlamps,\n"
        "blackout lamps and front corner markers glow warm white; the rear lamps glow\n"
        "red over white.\n"
        "\n"
