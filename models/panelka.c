...
typedef enum {
    ...
    // What was in the flats. Not structure, and not counted as any.
    FR_WARDROBE, FR_COUCH, FR_TABLE,
...
typedef enum {
    ...
    FC_CONTENTS,  // what was in the flats: it falls with the floor under it and lands on the pile, but is not made of the pile
...
static const FragClass FR_CLASS[FR_COUNT] = {
    ...
    // The joinery stays in the wall. Only what is *in* the frame bursts at the shock.
    //
    // Both used to go at the shock, and that put 210 window frames into the air a second before the wall they belonged to had moved: released at their standing height with only gravity on them, while the storeys around them were being driven down at 11 m/s, they were left hanging in a row above the collapsing mass for the length of the fall. Nothing was stuck -- measured at 1.88 s, 420 of 447 were still flying -- but a frame falling at g beside a building falling faster than g reads as a frame that has stopped.
    // A pane does blow out of its frame at a shock. A timber sash bolted into a reveal does not: it comes down with the panel it is set into, which is what the skin already does.
    [FR_J26P] = FC_SKIN, [FR_J26T] = FC_SKIN, [FR_J32P] = FC_SKIN, [FR_J32T] = FC_SKIN,
    [FR_J32BP] = FC_SKIN, [FR_J32BT] = FC_SKIN, [FR_JSTAIR] = FC_SKIN,
    [FR_JSTAIR2] = FC_SKIN, [FR_JDOOR] = FC_SKIN,
    ...
    [FR_WARDROBE] = FC_CONTENTS, [FR_COUCH] = FC_CONTENTS, [FR_TABLE] = FC_CONTENTS,
...
static const char *TYPE_NAME[FR_COUNT] = {
    ...
    [FR_WARDROBE] = "wardrobe", [FR_COUCH] = "couch", [FR_TABLE] = "table",
...
static void BuildStructureTypes(void)
{
    ...
    // What was in the flats.
    //
    // Three boxes, authored about the middle of their own footprint at floor level, in the plain painted material so that the colour is the per-instance tint: dark browns, greys and near-blacks, which is what a wardrobe, a couch and a kitchen table were in a flat like this.
    // They are invisible until the building opens, because the windows are opaque and there is nothing to see in through. What they are for is the pile, where a demolition throws out a great deal that is not concrete, and where 3241 pieces of grey plate had nothing among them that had ever belonged to anybody.
    Box(&gType[FR_WARDROBE].b[MAT_PAINT], -0.55f, 0.55f, 0.0f, 1.95f, -0.28f, 0.28f);
    Box(&gType[FR_COUCH].b[MAT_PAINT], -0.95f, 0.95f, 0.0f, 0.72f, -0.42f, 0.42f);
    Box(&gType[FR_TABLE].b[MAT_PAINT], -0.60f, 0.60f, 0.0f, 0.76f, -0.45f, 0.45f);
...
// What the flats were furnished in: dark brown veneer, brown, and the grey and near-black of a wardrobe nobody has opened in ten years.
// Authored very dark, because they are a tint on MAT_PAINT, whose own map is a 196 render grey, and because lighting.fs gamma-corrects on the way out: a 74 picked here came back at about 130, which is a beige.
static const Color FURNITURE[] = {
    { 34, 24, 17, 255 },
    { 44, 32, 21, 255 },
    { 26, 20, 15, 255 },
    { 20, 20, 21, 255 },
...
static void PlaceBuilding(void)
{
    for (int s = 0; s < SECTIONS; s++) {
        for (int f = 0; f < FLOORS; f++) {
            ...
            // What is in the rooms. A room is a bay wide, and runs from the spine to the inner face of the facade on its own side of the building; a piece is dropped inside it clear of all four walls, hashed off where the room is so the same room is always furnished the same way.
            for (int b = 0; b < BAYS; b++) {
                if (b == BAY_STAIR) continue;   // the stairwell is communal and empty
                float x = BayX(s, b);
                for (int side = 0; side < 2; side++) {
                    int room = ((s * FLOORS + f) * BAYS + b) * 2 + side;
                    int n = 1 + (int)(Hash2(room, 81, 4096, 907u) * 2.99f);
                    for (int k = 0; k < n; k++) {
                        static const FragType KIND[3] = { FR_WARDROBE, FR_COUCH, FR_TABLE };
                        FragType kind = KIND[(int)(Hash2(room, 82 + k, 4096, 911u) * 2.99f)];
                        float margin = 0.75f;
                        float rx = x + (Hash2(room, 85 + k, 4096, 919u) - 0.5f) * fmaxf(0.0f, BAY_W[b] - 2.0f * margin);
                        float depth = INNER_Z - SPINE_T * 0.5f - 2.0f * margin;
                        float rz = SPINE_T * 0.5f + margin + Hash2(room, 88 + k, 4096, 929u) * fmaxf(0.0f, depth);
                        EmitAt(kind, rx, y, (side == 0) ? rz : -rz, 360.0f * Hash2(room, 91 + k, 4096, 937u),
                               1.0f, FURNITURE[(int)(Hash2(room, 94 + k, 4096, 941u) * (float)COUNT_OF(FURNITURE) * 0.999f)]);
                    }
                }
            }
...
typedef struct {
    ...
    Vector3 v0;
    Vector3 acc;     // sideways acceleration through the fall; y is gravity's and is not kept here
    Vector3 axis;    // tumble axis, in the fragment's own frame
    Vector3 leanAxis; // horizontal, so that turning about it tips the piece away from the building
    float leanRate;  // radians of lean per metre the crush front has carried it down
    float lean;      // how far over it already was at the moment it let go
    float omega;
...
//
// A wall standing on a storey that is being destroyed does not descend rigidly and upright. It has no out-of-plane stability worth anything once the floors bracing it are gone -- a 0.30 m panel eleven metres wide is a card -- so it starts over as soon as its footing does. Riding the front down as a pure translation is what left a gable standing vertical and perfectly planar four metres into its own collapse, which is the one thing about this sequence nobody could look at and believe.
//
// Capped, because past about a third of a turn the piece is no longer leaning, it is falling, and by then its own release has come.
static float LeanAt(const Motion *m, float drop)
{
    return fminf(m->leanRate * drop, 1.05f);
}
...
static void PlanFragment(int i)
{
    ...
    if (wc.y < CUT_TOP) {
        ...
        // Plus a quarter of a second of its own. Every panel of a gable shares an x, so it shares a firing delay, and its release otherwise depends only on how high it is: twenty panels then let go on the same instant and the wall came away as one flat plane, standing perfectly coplanar beside a field of rubble.
        m->t0 += 0.42f + 0.26f * Rand01(i, 30);
        out = 0.18f + 0.030f * height + 0.35f * Rand01(i, 10);
    } else if (cls == FC_CONTENTS) {
        // It falls when the floor under it goes, tumbling far more than a slab does because it is light and has no bearing to break out of, and sliding outwards as the room opens.
        m->t0 += 0.10f * Rand01(i, 23);
        out = 0.6f + 1.4f * Rand01(i, 24);
        up = 0.2f * Rand01(i, 25);
        along = 1.4f * Rand11(i, 26);
    } else {
    ...
    // Sideways acceleration, which is the difference between a piece falling and a piece being pushed while it falls.
    //
    // Without it the horizontal motion is constant and every path is a straight line in plan: a facade peels as a sheet of parallel tracks, and a cross wall drops on a ruler. What actually pushes a piece sideways is the air and the debris being driven out of the storey collapsing under it, which is strongest near the middle of the block and outward from the spine, and which is why the two ends of a folded slab do not travel the same way.
    // It is not a wind field and does not pretend to be: a constant per fragment, so the path is a parabola in plan as well as in elevation, which is enough to bend the tracks apart. Glass and balconies are left out -- they leave before the storey under them is being crushed, so there is nothing yet to push them.
    if (cls == FC_SKIN || cls == FC_STRUCT || cls == FC_CONTENTS || cls == FC_FITTING) {
        // Which way sideways is depends on how high up the piece started, and the two ends have opposite signs.
        //
        // At the bottom the storeys are being crushed and the material in them is extruded outward: that is the same event the base surge in the dust is, and it throws the lowest debris furthest from the footprint.
        // At the top there is nothing above to push anything anywhere, and the perimeter underneath is peeling away, so the roof and the top storey lose their edge support and drop into the void rather than out of it. That is the fold reading inward at the top that a demolition has in every film of one, and without it the upper facade travels outward as fast as the rubble at the foot of the building, which it does not.
        // Where the sign changes is not something this model can derive, and 0.62 of the way up is chosen to put it a storey above the cut.
        float h = Clamp((wc.y - PLINTH_Y) / (ROOF_Y - PLINTH_Y), 0.0f, 1.0f);
        float profile = 1.0f - h / 0.62f;
        // Nearer the middle of the block the fold is deepest and there is the most air and debris being driven out of it.
        float mid = 1.0f - fabsf(wc.x) / (BLOCK_LEN * 0.5f);
        float push = (0.45f + 1.30f * mid) * (0.55f + 0.90f * Rand01(i, 27)) * profile;
        m->acc = (Vector3){ 1.5f * Rand11(i, 28), 0.0f, sz * push };
    }
    m->axis = Vector3Normalize((Vector3){ Rand11(i, 13) + 0.05f, Rand11(i, 14) * 0.4f, Rand11(i, 15) });

    // Which way it goes over while the crush is still carrying it: away from the spine, about a horizontal axis, so a facade panel leans out and a gable leans off the end of the block.
    Vector3 outward = (Vector3){ (fabsf(wc.x) > BLOCK_LEN * 0.5f - 1.0f) ? wc.x : 0.0f, 0.0f, wc.z };
    // A piece on the spine itself has no outward direction; give it one rather than a zero axis, which MatrixRotate would turn into NaN.
    if (Vector3Length(outward) < 1e-4f) outward = (Vector3){ 0.0f, 0.0f, 1.0f };
    outward = Vector3Normalize(outward);
    m->leanAxis = (Vector3){ outward.z, 0.0f, -outward.x };
    // The skin has nothing holding it and goes over fastest; the frame is tied at both ends and holds its line longer.
    m->leanRate = ((cls == FC_SKIN) ? 0.150f : (cls == FC_STRUCT) ? 0.055f : 0.0f)
                * (0.55f + 0.90f * Rand01(i, 31));
    m->omega = (glazed ? 3.4f : balcony ? 1.9f : cls == FC_CONTENTS ? 2.6f : cls == FC_STRUCT ? 1.25f : 0.7f) * (0.5f + Rand01(i, 16)) * (Rand01(i, 17) < 0.5f ? -1.0f : 1.0f);
...
// The lean, applied about a world-space axis through the piece's own centroid. It has to go on after the rest yaw, because that is the frame the axis was worked out in.
static Matrix ApplyLean(Matrix M, int i, float lean)
{
    const Fragment *f = &gFrag[i];
    const Motion *m = &gMotion[i];
    Vector3 wsc = Vector3Transform(Vector3Scale(m->c, f->scale), MatrixRotateY(f->yaw * DEG2RAD));
    M = MatrixMultiply(M, MatrixTranslate(-wsc.x, -wsc.y, -wsc.z));
    M = MatrixMultiply(M, MatrixRotate(m->leanAxis, lean));
    return MatrixMultiply(M, MatrixTranslate(wsc.x, wsc.y, wsc.z));
}

// Where a piece sits while the crush below is still carrying it: dropped by `drop`, and leaned over by however far that drop has taken it.
static Matrix RidePose(int i, float drop)
{
    const Fragment *f = &gFrag[i];
...
static Matrix PoseAtTau(int i, float tau)
{
    ...
    Vector3 d = {
        m->v0.x * tau + 0.5f * m->acc.x * tau * tau,
        m->v0.y * tau - 0.5f * GRAV * tau * tau,
        m->v0.z * tau + 0.5f * m->acc.z * tau * tau,
    };
    Vector3 sc = Vector3Scale(m->c, f->scale);

    // Scale, then swing about the fragment's own centroid, then take the rest yaw, then the lean it already had when it let go, then place.
    // Rotating about the local origin instead would spin a facade panel about its bottom inner corner, which is a hinge no broken panel has.
    // At tau = 0 the tumble and the ballistic step are both nothing, so this is exactly RidePose at the drop it let go from: the ride and the flight meet without a step, which they have to, because the piece is visible through both.
    Matrix M = MatrixScale(f->scale, f->scale, f->scale);
...
static void SolveLanding(void)
{
    ...
    for (int k = 0; k < n; k++) {
        ...
        wc = Vector3Transform(m->c, PoseAtTau(i, tau));
        e = ExtentAt(i, tau);
        // A wardrobe lands on the rubble and crushes flat. It takes its height from what is under it like everything else, and it adds nothing: counting a box that is nine tenths air as if it were concrete is exactly how the deleted rubble inflated this pile by a fifth.
        if (FR_CLASS[gFrag[i].type] == FC_CONTENTS) continue;
...
static void AddImpactDust(void)
{
    ...
    for (int i = 0; i < gFragCount; i++) {
        ...
        // Size is a stand-in for how much concrete arrived, so it only means that for concrete. A wardrobe is a cubic metre of chipboard and does not raise what a cubic metre of slab does.
        bool heavy = (vol >= 1.10f) && (FR_CLASS[gFrag[i].type] != FC_CONTENTS);
        if (!heavy && Hash2(i, 71, 4096, 743u) > 0.30f) continue;
...
static Matrix YardPose(int i, float t)
{
    ...
    if (pile > 0.35f) {
        ...
        if (a > 0.0f) {
            ...
            // How far over it goes. A lamp post lies down; a tree with a crown on it leans hard and stops, which is what it looks like afterwards and what keeps it visible: at 1.28 rad the crown of a birch came to rest 1.8 m off the ground, inside a pile cresting at 5.9, and the tree simply disappeared.
            float flat = (f->type == FR_BIRCH || f->type == FR_MAPLE) ? 1.10f : 1.48f;
            // Away from the building, and decided once by where the thing stands rather than by the lash.
            //
            // Taking the sign from the lash was a bug and a loud one. The lash is a damped oscillation at 0.62 Hz, so its sign flips every 0.8 s for as long as it rings; with `over` saturated the target was reached exactly, so a felled birch snapped through 2.2 radians -- 126 degrees -- twice a second, hurling its crown from one side of the pile to the other. Read as three separate faults from three frames of one run: the tree is invisible, the tree is outside the building, the tree is inside the building. One tree, one sign, three symptoms.
            tilt = Lerp(tilt, -flat, over);
            // Sunk into what buried it, but not so far that the trunk goes under with it.
            lift -= fminf(pile, 3.0f) * 0.30f * over;
        }
    }
...
static Group *PART_ROOMS[]  = { &gType[FR_WARDROBE], &gType[FR_COUCH], &gType[FR_TABLE] };
static Group *PART_DUST[]   = { &gDust, &gFlash };
static Group *PART_GROUND[] = { &gGround };

static BoundingBox bShell, bGlaz, bStruct, bRoof, bBalc, bEntry, bPlinth, bGround, bTrees, bCars, bYard, bRooms, bDust;
...
static void DrawRooms(void) { DrawGroups(PART_ROOMS, COUNT_OF(PART_ROOMS)); }
...
static BoundingBox RoomBounds(void) { return bRooms; }
...
static const Part PARTS[] = {
    ...
    { .name = "rooms", .draw = DrawRooms, .bounds = RoomBounds },
...
// A piece is drawn by two different functions in one cycle -- RidePose while the crush is still carrying it, PoseAtTau once it lets go -- and they have to agree exactly at the moment of handover or every fragment in the building pops as it releases.
//
// They agree by construction: at tau = 0 the tumble and the ballistic step are both nothing, and PoseAtTau is then the same scale, yaw, lean and drop that RidePose builds. By construction is not the same as measured, and the lean went into one of them before it went into the other.
static void CheckHandover(void)
{
    float worst = 0.0f;
    int worstAt = -1;
    for (int i = 0; i < gFragCount; i++) {
        if (!IsBuilding(gFrag[i].type)) continue;
        const Motion *m = &gMotion[i];
        Vector3 a = Vector3Transform(m->c, RidePose(i, m->drop));
        Vector3 b = Vector3Transform(m->c, PoseAtTau(i, 0.0f));
        float d = Vector3Distance(a, b);
        if (worstAt < 0 || d > worst) { worst = d; worstAt = i; }
    }
    TraceLog(LOG_INFO, "panelka: the ride and the flight meet to %.4f m at the worst fragment (%s)",
             worst, worstAt >= 0 ? TYPE_NAME[gFrag[worstAt].type] : "-");
    if (worst > 0.01f) {
        TraceLog(LOG_ERROR, "panelka: a fragment jumps %.3f m at the moment it lets go", worst);
    }
}
...
static void Init(void)
{
    ...
    bRooms = InstBounds(PART_ROOMS, COUNT_OF(PART_ROOMS), false);
...
const Scene SCENE = {
    ...
    .description =
        ...
        "There is one mesh per panel type and a list of transforms saying where the instances go, which is what makes the joint grid real rather than drawn on, and which is the decomposition the demolition will throw. 76 types and 3594 fragments.\n"
        ...
        "\n"
        "The flats are furnished. Three boxes -- a wardrobe, a couch and a table -- in dark brown, grey and near-black, one to three of them dropped inside every room that is not the stairwell, hashed off where the room is so the same room is always furnished the same way. 353 of them.\n"
        "Nothing sees them until the building opens, because the windows are opaque. They are for the pile, which was 3241 pieces of grey plate with nothing among them that had ever belonged to anybody. They fall when the floor under them goes, tumbling twice as fast as a slab because they are light and have no bearing to break out of, and they come to rest on the rubble like everything else -- but they deposit nothing into it. A wardrobe is chipboard and air and is crushed flat by the first slab that lands on it; counting its bounding box as concrete would have added about 390 m3 of material the building never had, which is more than the invented rubble that was just removed for exactly that reason. Their volume is likewise kept out of the figure the pile height is predicted from.\n"
        ...
        "It rides down, and goes over as it does. The crush front leaves the cut when the charges under that part of the block fire and climbs at 11 m/s on average, and while it is still below a piece that piece descends by 0.62 of the height that has gone underneath it, because that is what a crushed storey loses. Once the front has passed it there is nothing under it at all, so it is simply falling: it keeps the speed it had and gravity takes it from there.\n"
        "That second regime was missing for three revisions, and it is the one thing about this sequence that was reported twice from screenshots and diagnosed wrongly twice. The front was clamped to the piece's own height, so a piece descended while the front climbed to it and then stopped dead, in mid-air, until its own release -- 0.42 s later for the skin, of a three-second collapse. What that looks like is a row of window frames and wall panels stuck at third-floor height above a building that has already gone. The first diagnosis blamed the glazing for falling at g beside a building falling faster than g, which was true and was not this; the second moved the joinery into the skin, which was right on its own terms and changed nothing, because the skin was what was hanging.\n"
        "The speed the ride had reached is carried into the flight, or the piece stalls at the moment it lets go: the flight would otherwise start from rest and a panel coming down at 8 m/s would appear to stop and begin again. It also leans, away from the spine, at 0.15 rad per metre it is carried down for the skin and 0.055 for the frame, capped at 1.05. A wall standing on a storey that is being destroyed has no out-of-plane stability worth anything once the floors bracing it are gone -- a 0.30 m panel eleven metres wide is a card -- so it starts over as soon as its footing does. Riding the front down as a pure translation is what left a gable standing vertical and perfectly planar four metres into its own collapse. The ride and the flight are two different functions drawing the same piece in one cycle, so CheckHandover measures where they meet: at the moment of release they agree to 0.0000 m, which they have to, because a piece is visible through both. And CheckHover walks every fragment of the building at 25 ms over the whole cycle and reports the longest it hangs above two metres while moving slower than a metre a second, counting from its own charges rather than the first, because a piece the sequence has not reached yet is standing on an intact storey. It is 0.05 s, which is two samples at the moment of handover; it was 0.42 s and visible from across the courtyard.\n"
        "That check also needed one correction of its own before it could be believed. Written against FlightPose it reported a 1.92 s hover the render plainly does not have, because asking FlightPose for a time before release returns the pose the piece will have *at* release, which is stationary. The renderer and the check now call one BuildingPose, since a check that poses the piece differently from the renderer is measuring its own copy of the model.\n"
        ...
        "The front does not climb at one speed everywhere. It is the storeys below failing in turn, and how fast that runs depends on how much mass is standing on them: the middle of the block has a section leaning on it from either side and goes down fastest and deepest, at 1.2 of the average, while the two gable ends have a section on one side only and hold up longest, at 0.78. It is the same reason the middle of the pile is the highest part of it, and it spreads out when the last piece comes down.\n"
        ...
        "The pane is the exception to the crush front: every one in the building bursts at the shock rather than when its own storey is reached, spinning five times faster than a panel -- but leaving at 1.1 to 2.8 m/s, not the 3.6 to 7.8 it once had. The joinery it sits in does not. A pane blows out of its frame at a shock; a sash bolted into a reveal comes down with the panel it is set into. Sending both at the shock put 210 window frames into the air a second before the wall they belonged to had moved -- released at their standing height with only gravity on them while the storeys around them were being driven down at 11 m/s, and so left hanging in a row above the collapsing mass for the length of the fall. Nothing was stuck: measured at that moment, 420 of the 447 were still flying. A frame falling at g beside a building falling faster than g simply reads as a frame that has stopped. A balcony fails as its own storey is reached and 0.12 s after it, not 0.22 s before. The skin waits 0.42 s behind the structure at its own height, where a first attempt used 0.20, plus a quarter second of its own: every panel of a gable shares an x and so shares a firing delay, and its release otherwise depends only on how high it is, so twenty of them let go on the same instant and the wall came away as one flat plane standing perfectly coplanar beside a field of rubble.\n"
