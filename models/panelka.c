...
static void PlanFragment(int i)
{
    ...
    if (cls == FC_SKIN || cls == FC_STRUCT || cls == FC_CONTENTS || cls == FC_FITTING) {
        // Outward from the spine, and stronger the nearer the middle of the block, where the fold is deepest and the most air is being driven out.
        float mid = 1.0f - fabsf(wc.x) / (BLOCK_LEN * 0.5f);
        float push = (0.35f + 1.05f * mid) * (0.55f + 0.90f * Rand01(i, 27));
...
const Scene SCENE = {
    ...
    .description =
        ...
        "It flies, tumbling about its own centroid rather than its local origin, which would be a hinge no broken panel has. The drop it had already taken at its release time is exactly where the flight starts, so the two meet without a step. Not quite ballistically: it also carries a sideways acceleration through the fall, outward from the spine and strongest near the middle of the block, where the fold is deepest and the most air and debris are being driven out of the storey collapsing under it. Without that the horizontal motion is constant and every path is a straight line in plan -- a facade peels as a sheet of parallel tracks and a cross wall drops on a ruler. A constant per fragment is enough: the path is then a parabola in plan as well as in elevation and the tracks bend apart. Glass and balconies are left out, because they leave before there is any storey being crushed under them to push them. It costs a metre of throw, to 8.4 m clear of the footprint.\n"
