...
const Scene SCENE = {
    ...
    .description =
        ...
        "There is one mesh per panel type and a list of transforms saying where the instances go, which is what makes the joint grid real rather than drawn on, and which is the decomposition the demolition will throw. 60 types and 2787 fragments.\n"
        ...
        "Each panel is a cast core across its full width with an exposed-aggregate field standing 0.022 m proud of it and set back 0.170 m from every edge, so two neighbours leave 0.34 m of smooth cast render between them. That 0.170 is measured rather than chosen: scaling ref_05.jpg by the 2.66 m storey -- four sills 310 px apart -- gives 116.5 px/m, and the pale band between two panels reads 45 px both horizontally and vertically, so 0.386 m, or 0.19 a side, pulled in by a tenth because a blurred pale-to-dark edge measures wide. An earlier build used 0.045, and a 0.09 m band at the distance this scene is viewed from is a third of a pixel: it is the single reason that build rendered as an unbroken grey sheet with windows in it.\n"
        ...
        "Downpipes run at every section joint and gable corner on both facades, in one segment per storey so they break up with the building, and they are rusted through rather than painted, which is what ref_03 and ref_05 both show and the loudest colour on either facade.\n"
        "The gable is four panels across its 11.82 m depth, not two: ref_05 reads four columns across its end wall at the same border pitch the facade carries, and two 5.91 m slabs gave the gable a grid four times coarser than the rest of the building. It is blank, which ref_02, ref_03 and ref_07 show and ref_01 draws; ref_05 is the variant that does put two window columns in its gable, and ref_04 exists to show one painted instead.\n"
        "The roof is flat, with a 0.32 m upstand under a metal capping that oversails the facade by 0.11. The series specifies the original directly, as a flat non-ventilated roof finished in rolled bitumen; the pitched metal roofs in ref_02 and ref_07 are the re-roofing thousands of these were given in the 1990s.\n"
        "\n"
        ...
        "A slab and a cross wall are each laid as pieces the length of a room rather than as one plate the depth of the building. It costs no type and no geometry -- a piece authored about its own centre is placed twice and KNIT closes the two together -- and it is the pile it changes, where an earlier build stacked intact 5.7 m plates and 11.2 m cross walls.\n"
        "\n"
        "The yard.\n"
        ...
        "Glazing is the exception to the crush front: every pane in the building goes at the shock rather than when its own storey is reached, thrown 3.6 to 7.8 m/s and spinning five times faster than a panel. Balconies are the other: a cantilever with nothing above it, so it fails 0.22 s early, clamped so that nothing anywhere moves before the charges it is a consequence of.\n"
        "\n"
        "Where a piece comes to rest is decided by what is already there. The pile is a heightfield, and the fragments are planned in the order they land: each is put on top of whatever is in the cells its own footprint covers, and then deposits its own material -- volume by the divergence theorem, bulked by 1.55 -- spread over that footprint. 1967 m3 becomes 3049 of rubble and the packed pile crests at 6.6 m, against 4.4 m for a smooth mound of the same material; rigid pieces leave voids a smooth mound does not, and breaking the slabs and walls into room-length pieces widened that gap, which is the expected direction.\n"
